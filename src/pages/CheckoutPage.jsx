...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    // Only check for delivery location if we're actually checking out (not just viewing cart)
    if (cartState.checkout) {
      ...
      const postOrder = async () => {
        ...
        dispatch({ type: "CLEAR" });
        // Clear the delivery location from sessionStorage after order
        sessionStorage.removeItem('selectedDeliveryLocation');
      };
      postOrder();
    }
  }, [cartState.checkout, userInfo, navigate, dispatch]);

  const checkoutHandler = () => {
    // Navigate to delivery location page instead of directly checking out
    navigate('/delivery-location');
  };
